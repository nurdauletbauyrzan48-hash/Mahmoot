import { Answer, Player, Quizz } from "@mahmoot/common/types/game"
import { Server, Socket } from "@mahmoot/common/types/game/socket"
import { Status, STATUS, StatusDataMap } from "@mahmoot/common/types/game/status"
import { usernameValidator } from "@mahmoot/common/validators/auth"
import Registry from "@mahmoot/socket/services/registry"
import { createInviteCode, timeToPoint } from "@mahmoot/socket/utils/game"
import sleep from "@mahmoot/socket/utils/sleep"
  ...
  showResults(question: any) {
    ...
    const sortedPlayers = this.players
      .map((player) => {
        ...
        ;(player as any).correctCount =
          ((player as any).correctCount || 0) + (isCorrect ? 1 : 0)
    ...
    sortedPlayers.forEach((player, index) => {
      ...
      this.sendStatus(player.id, STATUS.SHOW_RESULT, {
        ...
        correctCount: (player as any).correctCount || 0,
        totalQuestions: this.quizz.questions.length,
        rank,
  ...
  showLeaderboard() {
    ...
    if (isLastRound) {
      ...
      this.broadcastStatus(STATUS.FINISHED, {
        subject: this.quizz.subject,
        top: this.leaderboard.map((p) => ({
          ...p,
          correctCount: (p as any).correctCount || 0,
          totalQuestions: this.quizz.questions.length,
        })),
      })
